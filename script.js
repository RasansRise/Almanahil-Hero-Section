(() => {
  ...
  function revealSwap(el, direction, { onSwap, delay = 0 } = {}) {
    const exitClass =
      direction === "next" ? "reveal-exit-left" : "reveal-exit-right";
  ...
  function updateContent(direction) {
    ...
    const titleExit =
      direction === "next" ? "title-exit-left" : "title-exit-right";

    const titleEnter =
      direction === "next" ? "title-enter-from-right" : "title-enter-from-left";
    ...
    swapField(originEl, car.origin, TITLE_EXTRA_DELAY);

    swapField(yearsEl, car.years, TITLE_EXTRA_DELAY);

    /* ---------- Description ---------- */

    revealSwap(descriptionEl, direction, {
      delay: TITLE_EXTRA_DELAY,
  ...
  function goTo(index, { restart = true } = {}) {
    ...
    const advance = (remaining) => {
      ...
      if (remaining > 1) {
        track(() => advance(remaining - 1), SLIDE_MS);
      } else {
        track(() => {
          isTransitioning = false;
        }, SLIDE_MS);
